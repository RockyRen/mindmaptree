...
// 拖动模块
const Drag = function (aNode: any, options: any = {}) {
  let node = aNode;
  ...
  //克隆的占位节点
  let cloneShape: any = null;
  //当前节点可添加的父节点BBox的集合
  let addableBoxSet: any = null;
  //最后一个重合的节点id
  let lastOverlapId: any = null;
  //当前重合的节点id
  ...
  // 取得克隆的shape：用于占位
  function _cloneNodeShape(node: any) {
  ...
  function _getAddableBBoxSet() {
    const addableBBoxSet: any = {};
    const addableSet = graph.getParentAddableNodeSet(node);

    forEach(addableSet, function (curNode: any) {
  ...
  // 将子节点及边设为透明
  function _setChildrenOpacity(children: any) {
  ...
  // 将透明子节点及边设为原样 
  function _setChildrenNormal(children: any) {
    forEach(children, function (child: any) {
  ...
  function moveFnc(dx: number, dy: number) {
    //如果不可渲染，则不可调用move
  ...
  return {
    setDrag: function () {
      if (!node.checkIsRootNode()) {
        ...
        node.shape.mousedown(function (event: any) {
          if (!enableRender.canRender) { return false; }
          viewportHandle.mousedownHandle(event);
        });
        node.shape.mousemove(function (event: any) {
          if (!enableRender.canRender) { return false; }
          viewportHandle.mousemoveHandle(event);
        });
        node.shape.mouseup(function (event: any) {
          if (!enableRender.canRender) { return false; }
